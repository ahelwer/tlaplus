...
public class Generator implements ASTConstants, SyntaxTreeConstants, LevelConstants, TLAplusParserConstants {
	...
	class Function {
		...
		boolean recursionCheck(UniqueString uniqueString) {
		  // Want to iterate starting from "bottom" of stack, which with Deque is
		  // done using a "descending" iterator going from tail to head.
		  final Iterator<pair> stackIter = this.funcStack.descendingIterator();
		  while (stackIter.hasNext()) {
		    final pair p = stackIter.next();
		...
		case N_GeneralId:
			...
			if ((sTreeNode.heirs()[1].getKind() == IDENTIFIER) && (sTreeNode.heirs()[1].getUS() == AtUS)
				...
				if (excStack.isEmpty() || excSpecStack.isEmpty()) {
					...
					return new AtNode((OpApplNode) excStack.peekFirst(), (OpApplNode) excSpecStack.peekFirst());
				}
			}
			;
